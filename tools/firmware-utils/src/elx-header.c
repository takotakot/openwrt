...
static int image_type = 0xFF;	/* default: NONE */
static int hw_id = 0;
...
static void add_hdr(unsigned char *buf, size_t data_len)
{
	MD5_CTX ctx;
	struct header header = {
		0,
		HOST_TO_BE32(image_type),
		"",
		HOST_TO_BE32(hw_id),
		"",
	...
	MD5_Init(&ctx);
	MD5_Update(&ctx, buf + sizeof(struct header), data_len);
	MD5_Final(header.md5sum, &ctx);

	memcpy(buf, &header, sizeof(struct header));
}

static void add_elecom1701_hdr(unsigned char *buf, size_t data_len)
{
	unsigned long crc;
	struct header_elecom1701 header = {
		ELECOM1701_ID,
		"",
		DEF_FW_VER,
	...
	crc = crc32(0, buf + sizeof(struct header_elecom1701), data_len);
	printf("crc32: %lx\n", crc);
	header.data_crc = HOST_TO_BE32(crc);

	memcpy(buf, &header, sizeof(struct header_elecom1701));
}
...
int main(int argc, char **argv)
{
		...
		case 't':
			...
			for (i = 0; i < sizeof(FIRMWARE_TYPES) / sizeof(firmware_type);
				...
				if (FIRMWARE_TYPES[i].id == tmp) {
					image_type = FIRMWARE_TYPES[i].id;
					printf("image type: %s (%d)\n",
						FIRMWARE_TYPES[i].name, FIRMWARE_TYPES[i].id);
				}
			}
			if (image_type == 0xFF) { /* if unmatched */
	...
	if (image_type != 6)
		hdr_len = sizeof(struct header);
	else
		hdr_len = sizeof(struct header_elecom1701);
	printf("hdr_len: 0x%lx\n", hdr_len);
	buf = malloc(hdr_len + data_len);

	size_t read = fread(buf + hdr_len, 1, data_len, in);
	if (read != data_len){
